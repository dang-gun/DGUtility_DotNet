...
namespace DGUtility.ProjectXml;
    ...
    public string SummaryGet(string sMemberName)
    {
        ...
        foreach (XElement[] item in this.Members)
        {
            ...
            {//검색된 내용이 있다.
                ...
                XElement? findXE_Summary = findXE.Element("summary");
                if (null != findXE_Summary)
                {//요약 정보가 있다.
                    sReturn = findXE_Summary.Value.Trim();
                }
                else
                {
                    findXE_Summary = findXE.Element("inheritdoc");

                    if (null != findXE_Summary)
                    {//inheritdoc가 있다.

                        //아직 상속받은 주석 찾는 방법을 모른다.
                        Debug.WriteLine("inheritdoc not find - ProjectXmlAssist.SummaryGet : " + sMemberName);
                    }
                }
